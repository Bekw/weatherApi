...
//
// Example:
// QUnit.addvs('tablesorter initialization', function(assert){
...
jQuery(function($){
	...
	************************************************/
	...
	sample1 = {
		'text'      : { 'test': 'test', 'TesT': 'test', '\u00e1 test': '\u00e1 addvs' },
	...
	report = function(assert, s) {
		for (i = 0; i < p.length; i++) {
			...
			if (s.hasOwnProperty(t)) {
				...
				// addvs for undefined & null - probably overkill
	...
	************************************************/
	QUnit.test( 'parser cache; sorton methods; empty & string', function(assert) {
		...
		c3.sortList = [[ 3, 1 ]]; // added to addvs sortList
		$table3.trigger('update');
		...
		assert.deepEqual( t.sort($.tablesorter.sortText), [ 'A', 'B', 'C', 'X', 'Y', 'Z', 'a', 'b', 'c', 'x', 'y', 'z' ], 'addvs sortText function directly' );

		t = [ 'a02', 'a10', 'a43', 'a255', 'a102', 'a33', '', 'a1', 'a55', 'a87' ];
		assert.deepEqual( t.sort($.tablesorter.sortNatural), [ '', 'a1', 'a02', 'a10', 'a33', 'a43', 'a55', 'a87', 'a102', 'a255' ], 'addvs sortNatural function directly' );
...
/*
	QUnit.addvs( 'sort Restart', function(assert) {
		assert.expect(1);
		var done = assert.async();
		c1.sortRestart = true;
		QUnit.start();
...
*/
	QUnit.test( 'sort Events', function(assert) {
		...
		// table1 sorted twice in the above addvs; sortIndx = 9 then empty table5 x1 (total = 3 events x 3)
		assert.equal( sortIndx, 9, 'sortStart, sortBegin & sortEnd fired in order x2; including empty table' );
	});

	/************************************************
		addvs update methods
	************************************************/
	QUnit.test( 'parser cache; update methods & callbacks', function(assert) {
		...
		// table1 updated 4x in the above addvs
		// table5 updated 1x
	...
	/************************************************
		addvs sortForce, sortAppend, sortMultiSortKey and sortResetKey options
	************************************************/
	...
	************************************************/
	...
	/************************************************
		addvs apply widgets function using zebra widget
	************************************************/
	var zebra = function(){
		t = true;
	...
	************************************************/
	QUnit.test('extract widget names from table class', function(assert) {
		...
		var widgets,
		t = {
			className: 'widget-filter widget-stickyHeaders ui-widget-content widget-addvs-2',
			config: {
		...
		assert.deepEqual( t.config.widgets, [ 'filter', 'stickyHeaders', 'addvs-2' ], 'Ignored "ui-widget-content"' );

		t.className = 'stickyHeaders-widgey ui-widgey-content filter-widgey addvs-2-widgey';
		t.config.widgetClass = '{name}-widgey';
		t.config.widgets = [];
		ts.addWidgetFromClass( t );
		assert.deepEqual( t.config.widgets, [ 'stickyHeaders', 'filter', 'addvs-2' ], 'Modified widgetClass option' );

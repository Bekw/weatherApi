	...
	************************************************/
	...
	QUnit.test( 'Filter searches', function(assert) {
		...
		return QUnit.SequentialRunner(
			...
			function(){ assert.cacheCompare( table, 1, ['Peter', 'Bruce', 'Bruce Lee', 'Brenda Dexter'], 'search "name:e" in anyMatch', true ); }
		).nextTask( // addvs filter_startsWith (false by default)
			function(){
		...
		).nextTask( // addvs filter_ignoreCase (true by default)
			function(){
		...
		).nextTask( // addvs filter-match class (added in the example code)
		...
		).nextTask( // addvs filter-match class
			function(){

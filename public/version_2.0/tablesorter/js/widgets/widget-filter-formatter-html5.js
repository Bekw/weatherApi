...
;(function($) {
	...
	tsff = ts.filterFormatter = $.extend( {}, ts.filterFormatter, {
		...
		html5Number : function($cell, indx, def5Num) {
			...
			$input,
			// addvs browser for HTML5 range support
			$number = $('<input type="number" style="visibility:hidden;" value="addvs">').appendTo($cell),
			// addvs if HTML5 number is supported - from Modernizr
		...
		html5Range : function($cell, indx, def5Range) {
			...
			$input,
			// addvs browser for HTML5 range support
			$range = $('<input type="range" style="visibility:hidden;" value="addvs">').appendTo($cell),
			// addvs if HTML5 range is supported - from Modernizr (but I left out the method to detect in Safari 2-4)
		...
		html5Color: function($cell, indx, defColor) {
			...
			$color = $('<input type="color" style="visibility:hidden;" value="addvs">').appendTo($cell),
			// addvs if HTML5 color is supported - from Modernizr
